...
// 				bindDetails.RawParameters = json.RawMessage(`{"postgresql_user": {"is_owner": true, "grant_privileges": []}}`)
// 				_, err := rdsBroker.Bind(ctx, instanceID, bindingID, bindDetails)
...
// 				bindDetails.RawParameters = json.RawMessage(`{"postgresql_user": {"revoke_privileges": []}}`)
// 				_, err = rdsBroker.Bind(ctx, instanceID, bindingID, bindDetails)
// 				Expect(err).To(MatchError(ContainSubstring(`postgresql_user.revoke_privileges makes no sense for owner`)))
// 				Expect(sqlProvider.GetSQLEngineCalled).To(BeFalse())
// 
// 				bindDetails.RawParameters = json.RawMessage(`{"postgresql_user": {"is_owner": true, "default_privilege_policy": "grant"}}`)
// 				_, err = rdsBroker.Bind(ctx, instanceID, bindingID, bindDetails)
